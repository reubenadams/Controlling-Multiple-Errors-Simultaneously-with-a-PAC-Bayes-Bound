...
def volume_estimates(emp_risk, m, M, delta, KL, sample):
    ...
    volume_intersection = num_in_intersection / sample_size
    volume_ours_only = num_in_ours_only / sample_size
    volume_theirs_only = num_in_theirs_only / sample_size
    volume_neither = num_in_neither / sample_size
...
def get_batch_volume_estimates(emp_risk, m, M, delta_B, delta_CI, KL, sample):
    ...
    individual_kls = get_batch_individual_kls(emp_risk.expand(sample_size, -1), sample)
    kl_vectors = get_batch_kl_vectors(emp_risk.expand(sample_size, -1), sample)
    ...
    vol_naive_CI = confidence_interval(num_positive=naive_flags.int().sum(), num_trials=sample_size, delta=delta_CI)
    vol_ours_CI = confidence_interval(num_positive=ours_flags.int().sum(), num_trials=sample_size, delta=delta_CI)
    vol_loose_intersection_CI = confidence_interval(num_positive=loose_intersection_flags.int().sum(),
                                                    num_trials=sample_size, delta=delta_CI)

    return naive_is_subset_of_ours, loose_intersection_is_subset_of_naive, vol_naive_CI, vol_ours_CI, vol_loose_intersection_CI


def get_all_bounds(M, m, delta_B, KL):
    naive_B_scalar_kl = get_naive_union_bound_on_scalar_kl(m, M, delta_B, KL)
    loose_naive_B_scalar_kl = get_naive_union_bound_on_scalar_kl(m, M, delta_B / 2, KL)
    our_B_vector_kl = get_our_bound_on_vector_kl(m, M, delta_B, KL)
    loose_our_B_vector_kl = get_our_bound_on_vector_kl(m, M, delta_B / 2, KL)
    return naive_B_scalar_kl, loose_naive_B_scalar_kl, our_B_vector_kl, loose_our_B_vector_kl


def get_num_in_vols(emp_risk, m, M, delta_B, KL, sample, all_bounds):
    sample_size = sample.shape[0]
    # naive_B_scalar_kl = get_naive_union_bound_on_scalar_kl(m, M, delta_B, KL)
    # print("Naive bound:", naive_B_scalar_kl)
    # loose_naive_B_scalar_kl = get_naive_union_bound_on_scalar_kl(m, M, delta_B / 2, KL)
    ...
    # print("Our bound:", our_B_vector_kl)
    # loose_our_B_vector_kl = get_our_bound_on_vector_kl(m, M, delta_B / 2, KL)
    naive_B_scalar_kl, loose_naive_B_scalar_kl, our_B_vector_kl, loose_our_B_vector_kl = all_bounds
    ...
    num_in_naive = naive_flags.int().sum()
    num_in_ours = ours_flags.int().sum()
    num_in_loose_intersect = loose_intersection_flags.int().sum()

    return num_in_naive, num_in_ours, num_in_loose_intersect


def get_CIs(num_in_naive, num_in_ours, num_in_loose_intersect, mc_samples, delta_CI):
    vol_naive_CI = confidence_interval(num_positive=num_in_naive, num_trials=mc_samples, delta=delta_CI)
    vol_ours_CI = confidence_interval(num_positive=num_in_ours, num_trials=mc_samples, delta=delta_CI)
    vol_loose_intersection_CI = confidence_interval(num_positive=num_in_loose_intersect, num_trials=mc_samples, delta=delta_CI)
    return vol_naive_CI, vol_ours_CI, vol_loose_intersection_CI


def get_vol_estimates(emp_risk, m, M, delta_B, delta_CI, KL, batch_size, num_batches):
    total_sample_size = batch_size * num_batches
    total_in_naive, total_in_ours, total_in_loose_intersect = 0, 0, 0
    all_bounds = get_all_bounds(M=M, m=m, delta_B=delta_B, KL=KL)
    print(all_bounds)
    for batch_num in range(num_batches):
        if batch_num % 100 == 0:
            print(f"\tMC batch {batch_num + 1} / {num_batches}")
        sample = unif_simplex_sample(M=M, num_samples=batch_size)
        n_naive, n_ours, n_loose_intersect = get_num_in_vols(emp_risk, m, M, delta_B, KL, sample, all_bounds)
        total_in_naive += n_naive
        total_in_ours += n_ours
        total_in_loose_intersect += n_loose_intersect
    print(f"\t\t\t\tNum in naive: {total_in_naive}/{total_sample_size}")
    print(f"\t\t\t\tNum in ours: {total_in_ours}/{total_sample_size}")
    print(f"\t\t\t\tNum in int: {total_in_loose_intersect}/{total_sample_size}")
    vol_naive_CI, vol_ours_CI, vol_loose_intersection_CI = get_CIs(total_in_naive, total_in_ours, total_in_loose_intersect, total_sample_size, delta_CI)
    return vol_naive_CI, vol_ours_CI, vol_loose_intersection_CI
...
def unif_simplex_sample(M, num_samples):
    ...
    return dist.sample((num_samples,)).cuda()
...
if __name__ == "__main__":
    ...
    M_vals = t.tensor([2**2, 5**2, 10**2])
    m_vals = t.tensor([100, 300, 1000])
    # m_vals = t.tensor([1000])
    # mc_samples = 100000
    ...
    for M in M_vals:
        # sample = unif_simplex_sample(M, mc_samples)
        emp_risk = t.tensor([1 / M for _ in range(M)], device="cuda")
        for m in m_vals:
            if m < M:
                continue
            print("M:", M)
            print("m:", m)
            vol_naive_CI, vol_ours_CI, vol_loose_intersection_CI = get_vol_estimates(
                emp_risk,
                m,
                M,
                delta_B,
